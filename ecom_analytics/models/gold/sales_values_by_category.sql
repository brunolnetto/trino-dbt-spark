...
WITH daily_sales_products AS (
  SELECT 
    ...
    product_id,
    ROUND(SUM(CAST(proportional_payment_value AS DECIMAL(10,2))), 2) AS sales,
    COUNT(DISTINCT order_id) AS orders,
    COUNT(*) AS items_sold,
    AVG(CAST(item_price AS DECIMAL(10,2))) AS avg_item_price
  FROM {{ ref('fact_sales') }}
  WHERE order_status_category = 'Completed'
...
monthly_category_totals AS (
  SELECT
    ...
    ROUND(SUM(sales) / NULLIF(SUM(items_sold), 0), 2) AS avg_revenue_per_item,
...
category_rankings AS (
  SELECT
    *,
    ...
    ROUND(
      (total_sales * 100.0) / NULLIF(SUM(total_sales) OVER (PARTITION BY monthly), 0), 
      2
    ) AS category_share_pct,
    ROW_NUMBER() OVER (PARTITION BY monthly ORDER BY total_sales DESC) AS category_rank
